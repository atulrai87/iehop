...
//		show_404($segments[0]);
	...
	function _parse_routes()
	{
		...
		foreach ($this->routes as $key => $val)
		{
			...
			if (preg_match('#^'.$key.'$#iu', $uri))
			{
				// Do we have a back-reference?
				if (strpos($val, '$') !== FALSE AND strpos($key, '(') !== FALSE)
				{
					$val = preg_replace('#^'.$key.'$#iu', $val, $uri);
				}
	 ...
	 * @return	string
	 */
	function fetch_class($check_for_admin_code = false)
	{
		if ($check_for_admin_code && $this->is_admin_class){
			return 'admin_'.$this->class;
		} elseif ($check_for_admin_code && $this->is_api_class){
