	...
	function _parse_function($function, $modifiers, $arguments)
	{
		switch ($function) {
			...
			case 'depends':
				...
				$CI = & get_instance();
				$_result = '';
				if(!$CI->pg_module->is_module_installed($_args['module'])) {
					$_result = '<?php /*';
				...
				if(!$this->_comment_start) {
					...
					for($i = 1;;$i++) {
						...
						if(empty($_args[$key]) ) {
							...
							if(!$CI->pg_module->is_module_installed($_args[$key])){
								$_result = '<?php /*';
								$this->_comment_start = true;
								break;
							}
...
//					$_output .= '->{(($var=$this->_TPL[\''.substr($var,3).'\']) && substr($var,0,2)!=\'__\') ? $_var : $this->trigger_error("cannot access property \\"$var\\"")}';
	...
	function _plugin_exists($function, $type)
	{
		...
		if (isset($this->_require_stack[$type . '.' . $function . '.php']) && function_exists('tpl_' . $type . '_' . $function))
		{
