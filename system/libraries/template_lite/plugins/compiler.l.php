...
function tpl_compiler_l($params, &$tpl)
{
	...
	if (isset($params['i'])) unset($params['i']);
	...
	if (isset($params['gid'])) unset($params['gid']);
	...
	$replace_array = 'array('.substr($replace_array, 1).')';

	$output = 'l('.$i.', '.$gid.', '.$lang_id.', '.$edit_type.', '.$replace_array.')';
