...
class Seo_model extends Model
{
	...
	private $_og_fields = array(
		...
		array('name'=>'og_description', 'type'=>'text'),
	);
	...
	public function generate_sitemap_xml($params){
		...
		if(strtotime($params['lastmod_date']) <= 0) $params['lastmod_date'] = date('Y-m-d H:i:s');
		...
		$output .= "\t\t<priority>".str_replace($search, $replace, ($params["priority"]) ? $params["priority"] : 0.1)."</priority>\r\n";
				...
				if(!empty($urls)){
					...
					foreach($urls as $url_data){
						...
						if($url_data["priority"]){
							...
							$output .= "\t\t<priority>".str_replace($search, $replace, $params["priority"])."</priority>\r\n";
						}
	...
	public function generate_sitemap_xml_cron(){
		$params = array();
		$params['changefreq'] = $this->pg_module->get_module_config('seo', 'sitemap_sitemap_changefreq');
		$params['lastmod'] = $this->pg_module->get_module_config('seo', 'sitemap_lastmod');
		$params['lastmod_date'] = date('Y-m-d H:i:s');
		$params['priority'] = $this->pg_module->get_module_config('seo', 'sitemap_priority');
