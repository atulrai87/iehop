	...
	protected function _post_message(){
		...
		if($im_status['im_on'] && $im_status['im_service_access']){
			...
			if($this->pg_module->is_module_installed('users_lists')){
				...
				$list_statuses = $this->Users_lists_model->get_statuses($this->user_id, $id_contact);
				// if you are in blacklist
				if(!empty($list_statuses['dest_user']['status_text']) && $list_statuses['dest_user']['status_text'] === 'block'){
					$post_access = false;
					$result['errors'][] = l('post_denied', 'im');
				// if user in your blacklist
				}elseif(!empty($list_statuses['user']['status_text']) && $list_statuses['user']['status_text'] === 'block'){
