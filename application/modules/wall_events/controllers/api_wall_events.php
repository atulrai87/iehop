	...
	private function _get_events($data, $use_load = false, $items_on_page = null, $page = null){
		...
		switch($data['place']){
			case 'myprofile':
				...
				break;
			case 'viewprofile':
				$is_friend = $is_lists_installed ? $this->Users_lists_model->is_friend($data['id_wall'], $this->user_id) : false;
			...
			default:
				...
				$friends_ids = array();
				if($is_lists_installed){
					$friends_ids = $this->Users_lists_model->get_friendlist_users_ids($this->user_id);
				}
				if($friends_ids){
					if($is_wall_owner){
		...
		if($result['count_all']){
			...
			if($wall_events){
				...
				//$result['html'] = trim($this->template_lite->fetch('wall_events'));
				$result['status'] = 1;
			}else{
				$result['all_loaded'] = 1;
			}
