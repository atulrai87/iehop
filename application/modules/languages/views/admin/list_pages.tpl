...
<div class="actions">
	<ul>
		<li><div class="l"><a href="{$site_url}admin/languages/pages_edit/{$current_lang_id}/{$current_module_id}">{l i='link_add_page' gid='languages'}</a></div></li>
		<li><div class="l"><a href="{$site_url}admin/languages/ajax_pages_delete/{$current_lang_id}/{$current_module_id}" onclick="javascript:  if(!confirm('{l i='note_delete_pages' gid='languages'}')) return false; delete_strings(this.href); return false;">{l i='link_delete_selected' gid='languages'}</a></div></li>
	</ul>
	&nbsp;
</div>
...
<div class="menu-level3">
	<ul>
		...
		<li class="{if $lang_id eq $current_lang_id}active{/if}"><a href="{$site_url}admin/languages/pages/{$lang_id}/{$current_module_id}">{$item.name}</a></li>
		{/foreach}
	</ul>
	&nbsp;
</div>
...
<tr>
	...
	<th>{l i='field_text' gid='languages'}</th>
	<th class="w50">&nbsp;</th>
</tr>
...
<tr{if $counter is div by 2} class="zebra"{/if} id="{$key}_tr">
	...
	<td>{$key}</td>
	<td class="editable" id="{$key}">{$item}</td>
	<td class="icons">
		<a href="{$site_url}admin/languages/pages_edit/{$current_lang_id}/{$current_module_id}/{$key}"><img src="{$site_root}{$img_folder}icon-edit.png" width="16" height="16" border="0" alt="{l i='link_edit_page' gid='languages'}" title="{l i='link_edit_page' gid='languages'}"></a>
		<a href="{$site_url}admin/languages/pages_delete/{$current_lang_id}/{$current_module_id}/{$key}" onclick="javascript: if(!confirm('{l i='note_delete_page' gid='languages' type='js'}')) return false;"><img src="{$site_root}{$img_folder}icon-delete.png" width="16" height="16" border="0" alt="{l i='link_delete_page' gid='languages'}" title="{l i='link_delete_page' gid='languages'}"></a>
	</td>
</tr>
{/foreach}
</table>
