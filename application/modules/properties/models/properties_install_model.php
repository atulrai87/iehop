...
class Properties_install_model extends Model
{
	...
	private $ausers_methods = array(
		...
		array( "module" => 'properties', 'method' => 'job_categories', 'is_default' => 0),
	);
	...
	*/
	...
	public function install_ausers_lang_update($langs_ids = null) {
		$langs_file = $this->CI->Install_model->language_file_read('properties', 'ausers', $langs_ids);
	...
	*/
	function _arbitrary_installing(){
		...
		$this->CI->load->model("properties/models/Job_categories_model");
		foreach ($this->CI->pg_language->languages as $id => $value){
			$this->CI->Job_categories_model->lang_dedicate_module_callback_add($value['id']);
		}
	...
	public function _arbitrary_lang_install($langs_ids) {
		...
		$this->CI->load->model('properties/models/Job_categories_model');
		$langs_file = $this->CI->Install_model->language_file_read('properties', 'demo_categories', $langs_id);

		foreach($langs_file as $category_id => $langs){
			$data = $this->CI->Job_categories_model->validate_job_category_langs($langs);
			$this->CI->Job_categories_model->save_job_category($category_id, $data);
		}
	}
	...
	public function _arbitrary_lang_export($langs_ids) {
		...
		/// categories
		$this->CI->load->model('properties/models/Job_categories_model');
		$demo_categories_return = $this->CI->Job_categories_model->export_job_categories_langs($langs_ids);

		return array( "demo" => $demo_return, "demo_categories" => $demo_categories_return );
	}
