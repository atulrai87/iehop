...
if (!defined('UPLOADS_TABLE')) define('UPLOADS_TABLE', DB_PREFIX.'uploads');
if (!defined('UPLOADS_THUMB_TABLE')) define('UPLOADS_THUMB_TABLE', DB_PREFIX.'uploads_thumb');
if (!defined('UPLOADS_WATERMARK_TABLE')) define('UPLOADS_WATERMARK_TABLE', DB_PREFIX.'uploads_watermark');
...
class Uploads_config_model extends Model
{
	...
	var $watermark_fields_all = array(
		'id',
		'gid',
		'name',
		'wm_type',
	...
	var $file_formats = array("jpg", 'gif', 'png');
	...
	var $watermark_test_image = "watermark_test.jpg";
	...
	function get_config_by_id($config_id){
		$data = array();
	...
	function get_config_by_gid($config_gid){
		$data = array();
	...
	function save_config($config_id, $data){
	...
	function validate_config($config_id=null, $data=array()){
	...
	function get_thumb_by_id($thumb_id){
		$data = array();
	...
	function get_config_thumbs($config_id){
		$data = array();
	...
	function save_thumb($thumb_id, $data){
		if (is_null($thumb_id)){
			$data["date_add"] = date("Y-m-d H:i:s");
	...
	function delete_thumb($thumb_id){
		...
		if(empty($thumb_data)) return false;
	...
	function validate_thumb($thumb_id=null, $data=array()){
		$return = array("errors"=> array(), "data" => array());
	...
	function get_watermark_by_id($wm_id){
		$data = array();
	...
	function get_watermark_by_gid($wm_gid){
		$data = array();
	...
	function format_watermark($data){
		if(empty($data["wm_type"])) $data["wm_type"] = 'text';
		if($data["wm_type"] == 'img'){
			if(!empty($data["img"])){
	...
	function save_watermark($wm_id, $data){
		if (is_null($wm_id)){
			$data["date_add"] = date("Y-m-d H:i:s");
	...
	function delete_watermark($wm_id){
	...
	function validate_watermark($wm_id=null, $data=array()){
		$return = array("errors"=> array(), "data" => array());
