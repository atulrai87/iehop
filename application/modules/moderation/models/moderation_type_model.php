...
class Moderation_type_model extends Model
{
	...
	function Moderation_type_model()
	{
		parent::Model();
	...
	function get_type_by_name($name){
		...
		if(!isset($this->types_cache["name"][$name]) || empty($this->types_cache["name"][$name])){
			$this->DB->select('id, name, mtype, view_link, edit_link, module, model, method_get_list, method_set_status, allow_to_decline, method_delete_object, method_mark_adult, template_list_row, date_add, check_badwords')->from(MODERATION_TYPE_TABLE)->where('name', $name);
	...
	function save_type($type_id, $attrs, $langs=array()){
