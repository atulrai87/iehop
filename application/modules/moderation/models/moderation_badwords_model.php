...
class Moderation_badwords_model extends Model
{
	...
	var $DB;
	var $CI;
	var $badwords_params_cache = array();

	function Moderation_badwords_model()
	{
		parent::Model();
	...
	function set_badword($word){
		...
		foreach($word_arr as $word){
			...
			if(!strlen($data["original"]) || !strlen($data["search"])) continue;
	...
	function get_similar_words($search_word){
	...
	function search_in_text($text){
		...
		//$this->DB->select("id, search")->from(MODERATION_BADWORDS_TABLE)->where_in("search", $text_array);
