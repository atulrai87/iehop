...
class Video_uploads_model extends Model
{
	...
	public function format_upload($config_gid, $postfix, $video, $image, $upload_type=''){
		$upload = array();
		if(!$video){
			return array();
		}
		...
		if($upload_type == 'local'){
			...
			$upload = array(
				...
				"embed" => $this->CI->Video_uploads_local_model->get_default_embed($url.$video, $config_data["local_settings"]["width"], $config_data["local_settings"]["height"])
			);
		}
	...
	public function upload_embed_video_image($video_config_gid, $data){
		...
		$path = $this->get_media_path($video_config_gid, $data['id_owner']);
		if (!is_dir($path)) {
			@mkdir($path, 0777, TRUE);
			@chmod($path, 0777);
		}
