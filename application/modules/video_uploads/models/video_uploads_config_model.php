...
if (!defined('VIDEOS_CONFIG_TABLE')) define('VIDEOS_CONFIG_TABLE', DB_PREFIX.'videos_config');
...
class Video_uploads_config_model extends Model
{
	...
	var $file_formats = array('avi', 'flv', 'mkv', 'wmv', 'asf', 'mpeg', 'mpg', 'mpe', 'qt', 'mov', 'movie', 'rv', 'mp4');
	var $default_img_formats = array('jpg', 'gif', 'png');
	...
	function get_config_by_id($config_id){
		$data = array();
		$result = $this->DB->select(implode(", ", $this->fields_all))->from(VIDEOS_CONFIG_TABLE)->where("id", $config_id)->get()->result_array();
		if(!empty($result)){
	...
	function get_config_by_gid($config_gid){
		$data = array();
		$result = $this->DB->select(implode(", ", $this->fields_all))->from(VIDEOS_CONFIG_TABLE)->where("gid", $config_gid)->get()->result_array();
		if(!empty($result)){
			$data = $this->format_config($result[0]);
		}
		return $data;
	...
	function get_config_list(){
		$data = array();
		$this->DB->select(implode(", ", $this->fields_all))->from(VIDEOS_CONFIG_TABLE)->order_by('gid ASC');
	...
	function get_config_count(){
		$this->DB->select(implode(", ", $this->fields_all))->from(VIDEOS_CONFIG_TABLE);
	...
	function save_config($config_id, $data, $upload_gid=''){
	...
	function validate_config($config_id, $data=array()){
		...
		if(isset($return["data"]["upload_type"]) && $return["data"]["upload_type"] == 'local'){
			if(isset($data["use_convert"])){
				$return["data"]["use_convert"] = intval($data["use_convert"]);
			}
		...
		if(isset($data["use_thumbs"])){
			$return["data"]["use_thumbs"] = intval($data["use_thumbs"]);
		}
		...
		if(isset($data["thumbs_settings"])){
			...
			foreach($data["thumbs_settings"] as $thumb){
				...
				$thumb["animated"] = intval($thumb["animated"]);
