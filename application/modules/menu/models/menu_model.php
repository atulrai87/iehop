...
if (!defined('MENU_TABLE')) define('MENU_TABLE', DB_PREFIX.'menu');
if (!defined('MENU_ITEMS_TABLE')) define('MENU_ITEMS_TABLE', DB_PREFIX.'menu_items');
...
class Menu_model extends Model
{
	...
	private $_menu_items_cache = array();
	...
	// menu functions
	public function get_menus_list($params=array(), $order_by=array()){
	...
	public function get_menu_items_list($menu_id, $check_permissions=false, $params=array(), $parent_id=0, $permissions=array()){
		...
		if(!isset($this->_menu_items_cache[$_key])){
			...
			if(!empty($results) && is_array($results)){
				...
				$this->_menu_items_cache[$_key] = $this->_generate_menu($parent_id);

			}else{
				$this->_menu_items_cache[$_key] = false;
			}
		}
		return $this->_menu_items_cache[$_key];
	}
	...
	public function _get_item_string_data($menu_id, $item_id, $lang_ids=array(), $type="value"){
		$data = array();
		if(empty($lang_ids)) $lang_ids = array_keys($this->CI->pg_language->languages);
	...
	public function _parse_link($item){
		...
		$params = explode("/", $link);
		if($params[0] == "admin"){
			$params[1] = (isset($params[1]) && !empty($params[1]) )?$params[1]:$this->CI->router->default_controller;
			$item["controller"] = "admin_".$params[1];
			$item["module"] = $params[1];
			$item["method"] = (isset($params[2]) && !empty($params[2]))?$params[2]:"index";
			$item["params"] = array_slice($params, 3);
			$is_admin = true;
		}else{
			$params[0] = (isset($params[0]) && !empty($params[0]) )?$params[0]:$this->CI->router->default_controller;
			$item["controller"] = $item["module"] = $params[0];
			$item["method"] = (isset($params[1]) && !empty($params[1]))?$params[1]:"index";
	...
	private function _generate_menu($parent_id){
		...
		foreach($this->temp_generate_raw_menu[$parent_id] as $subitem){
			if(isset($this->temp_generate_raw_menu[$subitem["id"]]) && !empty($this->temp_generate_raw_menu[$subitem["id"]])){
				$subitem["sub"] = $this->_generate_menu($subitem["id"]);
			}
			// Indicators
	...
	public function breadcrumbs_set_active($text, $url=''){
		// Cut
		if(mb_strlen($text, 'utf-8') > $this->_max_length + 3) {
			$text = mb_substr($text, 0, $this->_max_length, 'utf-8') . '...';
		}
	 ...
	 * @return array
	 */
	private function _get_lang_gids($menu_gid, $items_gids) {
	...
	public function export_langs($menus, $langs_ids = null) {
		...
		foreach($menus as $menu_gid => $menu_items) {
			$gids_db = $this->_get_lang_gids($menu_gid, $menu_items);
	...
	public function update_langs($menus, $langs_data) {
		foreach($menus as $menu_gid => $menu_items) {
			...
			foreach($menu_items as $item_gid) {
				...
				$menu_item = $this->get_menu_item_by_gid($item_gid);
				$this->CI->pg_language->pages->set_string_langs('menu_lang_' . $menu['id'],
																'menu_item_' . $menu_item['id'],
																$lang_data,
																array_keys($lang_data));
			}
		}
		return true;
	}
