...
if ( ! function_exists('get_admin_main_menu'))
{
	function get_admin_main_menu(){
		...
		$CI->template_lite->assign("menu", $menu_items);
		$html = $CI->template_lite->fetch("main_menu", 'admin', 'menu');
		echo $html;
	}
}
...
if ( ! function_exists('get_admin_level1_menu'))
{
	function get_admin_level1_menu($gid){
		...
		$CI->template_lite->assign("menu", $menu_items);
		$html = $CI->template_lite->fetch("level1_menu", 'admin', 'menu');
		echo $html;
	}
}
...
if ( ! function_exists('get_admin_level2_menu'))
{
	function get_admin_level2_menu($gid){
		...
		$CI->template_lite->assign("menu", $menu_items);
		$html = $CI->template_lite->fetch("level2_menu", 'admin', 'menu');
		echo $html;
	}
}
...
if ( ! function_exists('get_breadcrumbs'))
{
	function get_breadcrumbs($template=''){
		...
		if(!$template) $template = 'helper_breadcrumbs';

		//// add link to menu
		$breadcrumbs = $CI->Menu_model->get_breadcrumbs();

		$CI->template_lite->assign("breadcrumbs", $breadcrumbs);
		$html = $CI->template_lite->fetch($template, 'user', 'menu');
		return $html;
	}
}
...
{
	function linked_install_set_menu_item($gid, $menu_id, $type="create", $parent_id=0, $link='/', $status=1, $sorter=1, $indicator_gid = ''){
		$CI = & get_instance();
		$CI->load->model('Menu_model');
		if($type == 'create'){
			$item_data = array( 'gid' => $gid, 'menu_id' => $menu_id, 'parent_id' => $parent_id, 'link' => $link, 'status' => $status, 'sorter' => $sorter, 'indicator_gid' => (string)$indicator_gid );
			...
			$item = $CI->Menu_model->get_menu_item_by_gid($gid, $menu_id);
			if(!empty($item)) $CI->Menu_model->delete_menu_item($item['id']);
			return '';
		}else{
			return 0;
		}
		...
		if(!empty($items)){
			foreach($items as $item_gid => $item_data){
				if($process_type == "create"){
					$items[$item_gid]["id"] = linked_install_set_menu_item($item_gid, $menu_data['id'], $item_data["action"], $parent_id, $item_data["link"], $item_data["status"], $item_data["sorter"], $item_data["indicator_gid"]);
					...
					if(!$item_id) $items[$item_gid]["id"] = $item_id = linked_install_set_menu_item($item_gid, $menu_data["id"], "none");
