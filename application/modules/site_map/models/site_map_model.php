...
class Site_map_model extends Model
{
	...
	function get_sitemap_module_by_gid($module_gid){
	...
	function set_sitemap_module($module_gid, $data=array()){
	...
	function delete_sitemap_module($module_gid){
		$this->DB->where("module_gid", $module_gid);
	...
	function get_sitemap_xml_urls(){
		...
		$return = array(
			array(
				"url" => rewrite_link('site_map', 'index'),
				"priority" => 0.8
			),
		);
		return $return;
