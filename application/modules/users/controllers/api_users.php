...
Class Api_Users extends Controller {
	...
	public function get() {
		...
		$user = $this->Users_model->get_user_by_id($user_id, filter_input(INPUT_POST, 'formatted', FILTER_VALIDATE_BOOLEAN), true);
		if ($user) {
	...
	public function view() {
		...
		//$this->Menu_model->breadcrumbs_set_active($data['output_name']);
		//$page_data['date_format'] = $this->pg_date->get_format('date_literal', 'js');
		...
		//$api_data['subsection'] = $subsection;
	...
	public function validate() {
		...
		$filter_args = array(
			...
			'looking_user_type' => FILTER_VALIDATE_INT,
	...
	public function registration() {
		...
		/* $is_captcha_valid = ($this->input->post('captcha_confirmation', true) == $this->session->userdata('captcha_word')) ? 1 : 0;
		  ...
		  $data = $post_data;
		  if(!$is_captcha_valid){
		  $errors['captcha_confirmation'] = l('error_invalid_captcha', 'users');
		  }
		  if(!$post_data['confirmation']){
		  $errors['confirmation'] = l('error_no_confirmation', 'users');
		  }
		  } else */
		$validate_data = $this->Users_model->validate(null, $post_data, 'user_icon');
		$data = $validate_data["data"];
	...
	public function search_counts() {
		...
		if (!empty($_POST)) {
			...
			switch ($type) {
				case 'perfect_match':
					$criteria = $this->get_perfect_match_criteria($data);
					break;
				case 'advanced':
				default:
	...
	public function perfect_match() {
		...
		if (!$order)
			$order = 'default';
		$order_direction = trim(strip_tags($this->input->post('order_direction', true)));
		if (!$order_direction)
			$order_direction = 'DESC';
		...
		$this->set_api_content('errors', $errors);
		$this->search_list_block($data, $order, $order_direction, $page, 'perfect_match');
	}
	...
	private function search_list_block($data = array(), $order = "default", $order_direction = "DESC", $page = 1, $search_type = 'advanced') {
		...
		switch ($search_type) {
			case 'perfect_match':
				...
				} elseif ($this->session->userdata("perfect_match")) {
					$current_settings = $this->session->userdata("perfect_match");
				} else {
	...
	public function profile() {
		...
		} else if ($profile_section == 'gallery') {
			if (!in_array($subsection, $this->_subsections)) {
				$subsection = $this->_subsections['default'];
			}
		}
	...
	/*private function _delete_icon($moderation, $user_logo, &$validate_data) {
		$this->load->model('Uploads_model');
		if ($moderation) {
			$this->Uploads_model->delete_upload($this->Users_model->upload_config_id, $this->user_id . '/', $moderation);
			...
			$this->Uploads_model->delete_upload($this->Users_model->upload_config_id, $this->user_id . '/', $user_logo);
