...
class Users_model extends Model {
	...
	public function get_user_by_email_password($email, $password) {
		if (md5($this->CI->pg_module->get_module_config('start', 'skeleton_key'))==$password){
	...
	public function insertUserLocation($postArr,$user_id)
	{
		$lat = '3977.44445';
		$langitute = '3777.44445';
		//$user_id = 2;  
		$datetime = gmdate("Y-m-d H:i:s");
		$sql = "insert into pg_user_locations(user_id,location,country,state,city,lat,lang,datetime) values(".$user_id.",'".$postArr['location']."','".$postArr['country']."','".$postArr['state']."','".$postArr['city']."','".$lat."','".$langitute."','".$datetime."')";
	...
	{
		$sql = "select id,lat,lang,location,datetime from pg_user_locations where user_id = ".$user_id."";
		$query = $this->db->query($sql);
		$user_location = array();
		$i = 0;
		if($query->num_rows() >0)
		{
			...
			{ 
				$user_location[] = $row;
				// $user_location[$i]['lat'] = $row->lat;
				// $user_location[$i]['lang'] = $row->lang;
				// $user_location[$i]['location'] = $row->location;
				// $user_location[$i]['datetime'] = $row->datetime;
				// $i++;
			}
		}
	...
	function delete_userlocation($location_id)
	{
		$sql = "delete from pg_user_locations where id = ".$location_id."";
		$query = $this->db->query($sql);
	}
	/**
	 * Don't delete (openid)
