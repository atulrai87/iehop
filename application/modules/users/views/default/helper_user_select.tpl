...
<div id="user_select_{$select_data.rand}" class="user-select">
	...
	{$item.nickname} ({$item.output_name}){if $select_data.max_select ne 1}<br>{/if}<input type="hidden" name="{$select_data.var_name}{if $select_data.max_select ne 1}[]{/if}" value="{$item.id}">
	{/foreach}
	</span>
...
$(function(){
	loadScripts(
		"{/literal}{js module=users file='users-select.js' return='path'}{literal}",
		function(){
			new usersSelect({
